...
@ZenRegister
@Document("mods/lootoverhaul/loot/condition/PlayerHPCondition")
@ZenCodeType.Name("mods.lootoverhaul.loot.condition.PlayerHPCondition")
public class PlayerHPCondition extends LootCondition {
    ...
    public PlayerHPCondition(int num, LootProperty property){
        this(new IntProperty(num), property);
    }
    @ZenCodeType.Constructor
    public PlayerHPCondition(int min, int max, LootProperty property){
        this(new IntRangeProperty(min,max), property);
    }
    @ZenCodeType.Constructor
    public PlayerHPCondition(double num, LootProperty property){
        this(new DoubleProperty(num), property);
    }
    @ZenCodeType.Constructor
    public PlayerHPCondition(double min, double max, LootProperty property){
        this(new DoubleRangeProperty(min,max), property);
    }
    @ZenCodeType.Constructor
    public PlayerHPCondition(float num, LootProperty property){
        this(new FloatProperty(num), property);
    }
    @ZenCodeType.Constructor
    public PlayerHPCondition(float min, float max, LootProperty property){
        this(new FloatRangeProperty(min,max), property);
    }

    @ZenCodeType.Constructor
    public PlayerHPCondition(INumProperty pr, LootProperty property){
    ...
    public boolean isConditionSuccess(Entity entity) {
        if(entity instanceof Player player){
            ...
            if(min != Integer.MAX_VALUE && max != Integer.MAX_VALUE){
                return player.getHealth() >= min || player.getHealth() <= max;
            }
        }

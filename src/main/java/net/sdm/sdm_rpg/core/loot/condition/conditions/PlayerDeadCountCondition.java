...
public class PlayerDeadCountCondition extends LootCondition {
    ...
    public PlayerDeadCountCondition(){

    }

    @ZenCodeType.Constructor
    public PlayerDeadCountCondition(int num, LootProperty property){
        this(new IntProperty(num), property);
    }
    @ZenCodeType.Constructor
    public PlayerDeadCountCondition(int min, int max, LootProperty property){
        this(new IntRangeProperty(min,max), property);
    }
    @ZenCodeType.Constructor
    public PlayerDeadCountCondition(double num, LootProperty property){
        this(new DoubleProperty(num), property);
    }
    @ZenCodeType.Constructor
    public PlayerDeadCountCondition(double min, double max, LootProperty property){
        this(new DoubleRangeProperty(min,max), property);
    }
    @ZenCodeType.Constructor
    public PlayerDeadCountCondition(float num, LootProperty property){
        this(new FloatProperty(num), property);
    }
    @ZenCodeType.Constructor
    public PlayerDeadCountCondition(float min, float max, LootProperty property){
        this(new FloatRangeProperty(min,max), property);
    }

    @ZenCodeType.Constructor
    public PlayerDeadCountCondition(INumProperty pr, LootProperty property){
    ...
    public boolean isConditionSuccess(Entity entity) {
        if(entity instanceof ServerPlayer player) {
            if (min == Integer.MAX_VALUE && max != Integer.MAX_VALUE) {
                return player.getStats().getValue((Stat<?>) ObjectiveCriteria.DEATH_COUNT) >= max;
            }
            if (min != Integer.MAX_VALUE && max != Integer.MAX_VALUE) {
                return player.getStats().getValue((Stat<?>) ObjectiveCriteria.DEATH_COUNT) >= min || player.getStats().getValue((Stat<?>) ObjectiveCriteria.DEATH_COUNT) <= max;
            }
        }
    ...
    public ConditionsList getType() {
        return ConditionsList.PlayerDeadCount;
    }
}

...
@ZenRegister
@ZenCodeType.Name("mods.sdmrpg.loot.condition.NumberOfCreeperDetonationsCondition")
public class NumberOfCreeperDetonationsCondition extends LootCondition {
    ...
    public NumberOfCreeperDetonationsCondition(int num, LootProperty property){
        this(new IntProperty(num), property);
    }
    @ZenCodeType.Constructor
    public NumberOfCreeperDetonationsCondition(int min, int max, LootProperty property){
        this(new IntRangeProperty(min,max), property);
    }
    @ZenCodeType.Constructor
    public NumberOfCreeperDetonationsCondition(double num, LootProperty property){
        this(new DoubleProperty(num), property);
    }
    @ZenCodeType.Constructor
    public NumberOfCreeperDetonationsCondition(double min, double max, LootProperty property){
        this(new DoubleRangeProperty(min,max), property);
    }
    @ZenCodeType.Constructor
    public NumberOfCreeperDetonationsCondition(float num, LootProperty property){
        this(new FloatProperty(num), property);
    }
    @ZenCodeType.Constructor
    public NumberOfCreeperDetonationsCondition(float min, float max, LootProperty property){
        this(new FloatRangeProperty(min,max), property);
    }

    @ZenCodeType.Constructor
    public NumberOfCreeperDetonationsCondition(INumProperty pr, LootProperty property){
    ...
    public boolean isConditionSuccess(Entity entity) {
        if(min == Integer.MAX_VALUE && max != Integer.MAX_VALUE){
            return LevelInfo.creeperDetonation >= max;
        }
        if(min != Integer.MAX_VALUE && max != Integer.MAX_VALUE){
            return LevelInfo.creeperDetonation >= min || LevelInfo.creeperDetonation <= max;
        }

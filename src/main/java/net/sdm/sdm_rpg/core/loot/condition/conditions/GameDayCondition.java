...
@ZenRegister
@Document("mods/lootoverhaul/loot/condition/GameDayCondition")
@ZenCodeType.Name("mods.lootoverhaul.loot.condition.GameDayCondition")
public class GameDayCondition extends LootCondition {
    ...
    public GameDayCondition(int num, LootProperty property){
        this(new IntProperty(num), property);
    }
    @ZenCodeType.Constructor
    public GameDayCondition(int min, int max, LootProperty property){
        this(new IntRangeProperty(min,max), property);
    }
    @ZenCodeType.Constructor
    public GameDayCondition(double num, LootProperty property){
        this(new DoubleProperty(num), property);
    }
    @ZenCodeType.Constructor
    public GameDayCondition(double min, double max, LootProperty property){
        this(new DoubleRangeProperty(min,max), property);
    }
    @ZenCodeType.Constructor
    public GameDayCondition(float num, LootProperty property){
        this(new FloatProperty(num), property);
    }
    @ZenCodeType.Constructor
    public GameDayCondition(float min, float max, LootProperty property){
        this(new FloatRangeProperty(min,max), property);
    }

    @ZenCodeType.Constructor
    public GameDayCondition(INumProperty pr, LootProperty property){
    ...
    public boolean isConditionSuccess(Entity entity) {
        if(min == Integer.MAX_VALUE && max != Integer.MAX_VALUE){
            return entity.level().getGameTime() % 24000 >= max;
        }
        if(min != Integer.MAX_VALUE && max != Integer.MAX_VALUE){
            return entity.level().getGameTime() % 24000 >= min || entity.level().getGameTime() % 24000 <= max;
        }
